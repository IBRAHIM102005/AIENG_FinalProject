...
async def fetch_selected_sources(
    ...
    *,
    ...
    Results are served from the TTL-aware CacheService when available.
    ...
    cache: CacheService[list[Source]] = CacheService()
        ...
        try:
            ...
            if use_cache:
                ...
                if cached is not None:
                    ...
                    logger.info(
                        "source_cache_hit source=%s count=%s elapsed=%.3f",
                        name, len(cached), elapsed,
                    )
                    return name, cached, elapsed
            ...
            # --- Cache store ---
            if use_cache:
                await cache.set(name, question, result)
    ...
    raw_results = await asyncio.gather(
        *(run_one(name) for name in selected),
    ...
    for item in raw_results:
        if isinstance(item, BaseException):
            # Unexpected exception escaping run_one — log but continue
            logger.error("gather_task_failed error=%s", item)
            continue
