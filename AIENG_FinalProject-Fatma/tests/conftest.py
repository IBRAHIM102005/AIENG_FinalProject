...
Fixture catalogue
-----------------
db_store            — in-memory CacheStore, open for the test, auto-closed.
repo                — SessionRepository wired to db_store.
mock_repo           — MagicMock(spec=SessionRepository) for unit tests.
cache_store_path    — tmp_path-based file CacheStore (tests real file I/O).
sample_session      — A "done" ResearchSession already persisted in db_store.
pending_session     — A "pending" ResearchSession already persisted in db_store.
fake_wiki_sources   — Two canned Wikipedia Source objects.
fake_arxiv_sources  — Two canned arXiv Source objects.
sample_sources      — Three Sources from three different origins.
sample_answer       — AnswerWithCitations built from sample_sources.
fake_llm            — Offline FakeLLM stub (records calls, no API).
fake_web            — Offline FakeWebSearch stub (no HTTP).
mock_fetch_wikipedia — AsyncMock returning fake_wiki_sources.
mock_fetch_arxiv    — AsyncMock returning fake_arxiv_sources.
mock_fetch_web      — AsyncMock returning web-only sample_source.
event_loop          — Session-scoped asyncio loop for pytest-asyncio.
...
def event_loop():
    ...
    asyncio.AbstractEventLoop
        New event loop, closed after the session finishes.
    """
